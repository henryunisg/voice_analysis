...
def calculate_significant_results():
    ...
    # CONFIGURATION
    # ───────────────────────────────────────────────────────────────────────────
    CSV_FILE = "voice_analysis_female.csv"
    TARGET = "selected"
    EVENT_COL = None  # "event"
    SOURCE_COL = "source"
    CONTROLS = ['age', 'race', 'dress_formality', 'attractiveness'] #'gender'
    ...
    sig_df.to_csv("significant_results_female.csv", index=False)
    print(f"{len(sig_df)} significant terms @ p ≤ {P_TARGET}")
    print("Saved to: significant_results_female.csv")
...
def get_descriptive_statistics(model_type="full"):
    ...
    if model_type == "full":
        ...
        cname = 'mean_pitch_[0.00, 1.00)_dn_mean'
        raw = df[cname]
...
if __name__ == '__main__':
    ...
    print_glm_stats(calculate_sample(model_type="full"), calculate_sample(model_type="null"))
